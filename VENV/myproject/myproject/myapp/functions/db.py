    ...
    def fetch(self, attrs = [], values = [], method = 1, keys = None, dic =None):
        ...
        for i in range(len(attrs)):
            ...
            if method == 1:
                ...
                return [x for x in keys if dic[x][attrs[i]] == values[i]]
            elif method == 2:
                #containment mode
                #print('filtering: %s in %s' % (values[i], attrs[i]))
                return [x for x in keys if values[i] in dic[x][attrs[i]]] 
            else:
                #re mode
                #print('filtering: %s match %s' % (attrs[i], values[i]))
                return [x for x in keys if re.match(values[i] , dic[x][attrs[i]])]  
...
class SCH_db(db):
    ...
    def get_imp_type_key(self):
        return self._imp_type_key
    def set_imp_type_key(self, value):
        self._imp_type_key = value
        return
    ...
    def _comp_add(self, key, value):
        if key in self._comp_ref:
            ...
            #print(value)
            self._comp_ref[key] = {self.get_page_key(): value[0], self.get_design_key(): [value[1]], self.get_imp_type_key(): value[2]}
        return

    def op_check(self, op_value, keys):
        res = []
        for op in self.get_op_name():
            res += self.fetch(attrs = [op], values = [op_value], method = 3, keys = keys)
    ...
    def parser(self, sku = ''):        
        ...
        if self._comp_file:
            ...
            try:
                with open(self._comp_file) as f:
                    ...
                    #print(head)
                    if head.split('\t')[:4] != ['Reference','', 'Designator', 'Implementation Type']:
                        print('Undefined input: %s'%head.split('\t')[:4])
                        print('change it to -->', 'Reference','', 'Designator', 'Implementation Type')
                        return
                    occ_count = 0
                    # useless. just for avoiding compiler error
                    inst_key = page = design = 'Schematic View'
                    s_flag = False
                    for line in f:
                        temp = line.replace('\n', '').split('\t')
                        
                        if temp[1][0] != '/':
                            if occ_count == 0 and s_flag:
                                self._comp_add(inst_key, [page, design, imp_type])
                            ...
                            self._comp_add(temp[0], [page, temp[2], imp_type])
                    if occ_count == 0:
                        self._comp_add(inst_key, [page, design, imp_type])
        ...
        try:
            ...
            with open(self._file_name, 'r') as f:
                ...
                # 'Implementation Type' in attributes if hierarchical block is used
                try:
                    hierarchical_index = self._attrs('Implementation Type')
                ...
                for i in f:
                    ...
                    if self.SCH_type == 'PARTOCC' and self._comp_file:
                        ...
                        if (values[PR_flag][:-1] in self._comp_ref
                            ...
                            #self.clear()
                            return
                        #ignore hierarchical block
                        if self._comp_ref[key][self.get_imp_type_key()]:
                            continue
                    ...
                    for op_index in op_flag:
                        op = (not re.match(option_str, values[op_index])) or sku == values[op_index]
                        if not op:
                            break
                    values.append(op)
