...
##      self.output_print()
        ...
        self.unset_op[0].sort()
        self.unset_op[1].sort()
        self.set_op[0].sort()
        self.set_op[1].sort()
        part_BOM_lst.sort()
...
##      #print(self.unset_op);
    ...
    def check_comp(self, check_type,fun_flag, key, SCH_attr, BOM_attr):
        SCH = self.SCH.get_from_key(key)
        SCH_value = SCH[SCH_attr]
        BOM = self.BOM.get_from_key(self.BOM.get_from_location(key.upper()))
        BOM_value = BOM[BOM_attr]
        if check_type == 'res' and fun_flag == 'Value':
            return self.res2value(BOM_value) == self.res2value(SCH_value)
        elif fun_flag == 'PCB Footprint':
            return BOM_value != '' and BOM_value in SCH_value
        elif fun_flag == 'Part Number':
##                        return SCH_value == BOM_value or SCH_value in BOM['SUB']
            return SCH_value in BOM['SUB']
        else:
            return BOM_value == SCH_value
    ...
    def check_difference(self, check_type, flag='cn', except_R_C = False):  
        ...
        #count = 0
        return(sorted(res, key = lambda x: self.SCH.get_from_key(x)['Part Number']),
               incor
               )
