try:
        ...
        import myproject.myapp.db as db
        from myproject.myapp.SCH_NET import *
...
class ASRR:
	...
	def output_append(self,*arg):
		...
		for i in arg:
			temp += str(i)+'\t'
		self._output += temp[:-1]+'\n'
		#self._output += temp[:-1]+'<br>'
		return
	
	def output_clear(self):
		#self._output = '%s >>>>><br><br>'%datetime.datetime.now().strftime('%y-%m-%d  %H:%M:%S')
	...
	def optional_show(self):
		...
		print("Incorresponding part:")
		print(self.set_op[0]);
		print("Incorresponding part:")
		print(self.set_op[1]);
		print("2. set option to 'N/A' :")
		print("Incorresponding part:")
		print(self.unset_op[0]);
		print("Incorresponding part:")
		print(self.unset_op[1]);
	...
	def check_difference(self, check_type, flag='cn'):	
		...
		SCH_keys = self.SCH.fetch(['Part Number'], [type_flag], method = 3, keys = self.part_both)
	...
	def correct(self, cor_type):
		...
		both_key = self.SCH.fetch(['Part Number'], [type_flag], method = 3, keys = self.part_both)
		for SCH_key in both_key:
			SCHpart = self.SCH.get_from_key(SCH_key)
			BOM_key = self.BOM.get_from_location(SCH_key.upper())
			BOMpart = self.BOM.get_from_key(BOM_key) 
			if re.match(type_flag, BOMpart['Part Number']):
				#type of part in SCH and BOM should be corresponding
				if BOMpart['Part Number'] != SCHpart['Part Number']:
					count += 1
					for attr in attrs:
						if attr in BOMpart:
				...
				print('%s is not a %s element in BOM' %(SCH_key, cor_type))
		print('-'*100)
